...
//   port x SIM v3.5revC (the first apples-to-apples sim comparison)
void cmp_prodclus()
{
  ...
  TFile*fs=TFile::Open("prodclus_v35.root");      TTree*sc=(TTree*)fs->Get("ntp_clus");
  ...
  auto d3=[&](TVirtualPad*p,TH1D*a,TH1D*g,TH1D*b,const char*ti,bool logy){
    ...
    L->AddEntry(b,"port #times SIM v3.5revC","l"); L->Draw(); };

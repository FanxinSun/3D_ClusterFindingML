void pau_final(){
  ...
  TFile*fs=TFile::Open("digi_frames_production_v32.root"); TTree*sh=(TTree*)fs->Get("ntp_hit");
  TFile*fri=TFile::Open("island91_real.root"); TTree*ri=(TTree*)fri->Get("ntp_cluster");
  TFile*fsi=TFile::Open("island91_frames_production_v32.root"); TTree*si=(TTree*)fsi->Get("ntp_cluster");
  ...
  auto d2=[&](TVirtualPad*p,TH1D*a,TH1D*b,const char*ti,bool logy){
    ...
    L->AddEntry(a,"REAL","l"); L->AddEntry(b,"SIM pAu frames v2","l"); L->Draw(); };
  ...
  L6.AddEntry(i6,"REAL","l"); L6.AddEntry(j6,"SIM pAu frames v2","l"); L6.Draw();
  c2.SaveAs("/home/rog/sPHENIX/3D_ClusterFindingML/sim_validation_plots/pau_islands.png");

  // ---- residual table ----
  printf("\n== CORRECTED residual table (real vs pAu frames v2 @ 390 kHz) ==\n");

...
LIBS="raw_lib_angthin_j3_0.root,raw_lib_angthin_j3_1.root,raw_lib_angthin_j3_2.root,raw_lib_angthin_j3_3.root"
EVALS="eval_angthin_0.root,eval_angthin_1.root,eval_angthin_2.root,eval_angthin_3.root"
FLASH="raw_lib_cmflash_w.root"
RSPEC="363:1,619:1,641:1,652:1,652:1,653:1,656:1,657:1,663:1,669:1"
MBD="angthin_mbd.txt|ang_run_0.dat,ang_run_1.dat,ang_run_2.dat,ang_run_3.dat"
...
for i in $(seq 0 $((NB-1))); do
  ...
  tpc_readout(\"fP_$i.root\",\"dP_$i.root\",1.03,20.0,1,1,4711,\"$DM\",11.0,0.39,0.55,1.0,0.021,7.0,36.0,70.0,11.0,940.0,2,0.29,10.0,1.24,1.06,-1.0,5.0,1.0);" 2>&1 | grep -E "frame_composer: $PER|rate envelope|tpc_readout: fP"
  root -l -b -q "islandize91.C+(\"dP_$i.root\",\"i91_$i.root\",1,\"\",\"fP_$i.root\")" 2>&1 | grep "islandize91: dP"
done
hadd -f frames_pau_production_v40rt.root fP_*.root > /dev/null 2>&1
hadd -f digi_frames_production_v40rt.root dP_*.root > /dev/null 2>&1
hadd -f island91_frames_production_v40rt.root i91_*.root > /dev/null 2>&1
rm -f fP_*.root dP_*.root i91_*.root
root -l -b -q -e 'gROOT->ProcessLine(".L islandize.C+"); islandize("digi_frames_production_v40rt.root","island_frames_v40rt.root",1);' 2>&1 | grep islandize
echo "=== V4.0RT PRODUCTION COMPLETE ==="

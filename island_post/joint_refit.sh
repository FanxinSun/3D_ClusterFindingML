...
for CL in 0.08 0.10 0.12; do
  LIB="raw_scan_cl$CL.root"; [ "$CL" = "0.12" ] && LIB="raw_lib_pauLa.root"
  root -l -b -q -e "
  gROOT->ProcessLine(\".L frame_composer.C+\");
  frame_composer(\"$LIB\",\"f_j.root\",30,275.,20260910,0,\"\");" > /dev/null 2>&1
  for G in 0.72 0.78 0.84; do
    root -l -b -q -e "
    gROOT->ProcessLine(\".L tpc_digitize.C+\");
    tpc_readout(\"f_j.root\",\"d_j.root\",$G,20.0,1,1,4711,\"$DM\",11.0,0.39,0.0,0.0005,0.025,7.0);" > /dev/null 2>&1
    ...
    // mid/high pixel tail fraction (the user panels): frac adc in 200-800
    double mid=hx.Integral(hx.FindBin(200),hx.FindBin(800))/hx.Integral();
    printf(\"cl=$CL g=$G: pixm %.1f mid200-800 %.4f band %.3f | phi %.2f cliff %.3f | zg %.2f/%.2f | size %.2f px %.0fk\n\",
      hx.GetMean(), mid, band, hp.GetMean(), m2060,
      pz.GetBinContent(pz.FindBin(3000)), pz.GetBinContent(pz.FindBin(5500)),
      hs.GetMean(), h->GetEntries()/30./1e3);" 2>&1 | grep "cl="
  done
done
echo "TARGETS: pixm 99.15 mid .0083(real) band 0.233 | phi 3.41 cliff 0.251 | zg 6.97/10.35 | size 10.7 px ~230k(single-lib est)"
echo "JOINT REFIT DONE"

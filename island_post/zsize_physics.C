...
// Sim side follows the CURRENT production (v3.6 via island_frames_v40b.root).
void zsize_physics(){
  ...
  TFile*fv=TFile::Open("island_frames_v40b.root"); TTree*si=(TTree*)fv->Get("island");
  ...
  L2->AddEntry(d,"REAL islands","l"); L2->AddEntry(e,"SIM v4.0","l"); L2->Draw();
  c.cd(3);
  ...
  L3->AddEntry(p1,"REAL islands","l"); L3->AddEntry(p2,"SIM v4.0","l"); L3->Draw();
  c.SaveAs("/home/rog/sPHENIX/3D_ClusterFindingML/sim_validation_plots/zsize_physics.png");
  printf("saved zsize_physics.png (v3.6)\n");
}

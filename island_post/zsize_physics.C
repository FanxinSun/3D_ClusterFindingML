...
//  (2) real vs pAu-v2 islands zsize (uncapped physics tails);
//  (3) diffusion growth: mean island z-size vs |z| (drift length), real vs v2.
void zsize_physics(){
  ...
  TFile*fv=TFile::Open("island_frames_v32.root"); TTree*si=(TTree*)fv->Get("island");
  ...
  L2->AddEntry(d,"REAL islands","l"); L2->AddEntry(e,"SIM pAu frames v2","l"); L2->Draw();
  c.cd(3);
  ...
  L3->AddEntry(p1,"REAL islands","l"); L3->AddEntry(p2,"SIM pAu frames v2","l"); L3->Draw();
  c.SaveAs("/home/rog/sPHENIX/3D_ClusterFindingML/sim_validation_plots/zsize_physics.png");
  printf("saved zsize_physics.png (pAu v2)\n");
}

#!/usr/bin/env bash
# P3 production v3.6 revE (2026-07-14): LEG (b)+(c) TRANSPORT PASS.
...
RSPEC="158:1,269:1,279:1,283:1,283:1,284:1,285:1,286:1,289:1,291:1"
...
for i in $(seq 0 $((NB-1))); do
  ...
  frame_composer(\"$LIBS\",\"fP_$i.root\",$PER,275.,2026091$i,$((i*PER)),\"$EVALS\",\"$FLASH\",0.44,1.0,\"$SPEC\",1.5,2.0,1.1,0.75,\"$RSPEC\",1.0,\"$MBD\",0.56,0.0,\"$ENV\");
  gROOT->ProcessLine(\".L tpc_digitize.C+\");
  tpc_readout(\"fP_$i.root\",\"dP_$i.root\",0.86,20.0,1,1,4711,\"$DM\",11.0,0.39,0.55,0.87,0.021,7.0,36.0,70.0,11.0,940.0,2,0.29,10.0,1.24,1.06,-1.0,5.0,1.0);" 2>&1 | grep -E "frame_composer: $PER|rate envelope|tpc_readout: fP"
...
echo "=== V3.6revE PRODUCTION COMPLETE ==="

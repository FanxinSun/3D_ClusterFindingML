#!/usr/bin/env bash
# P3 production v3.6 revC (2026-07-14): TRIGGER CURRENCY FIX + empirical envelope.
...
for i in $(seq 0 $((NB-1))); do
  ...
  frame_composer(\"$LIBS\",\"fP_$i.root\",$PER,275.,2026091$i,$((i*PER)),\"$EVALS\",\"$FLASH\",0.44,1.0,\"$SPEC\",1.5,2.0,1.1,0.75,\"$RSPEC\",1.0,\"$MBD\",0.37,0.0,\"$ENV\");
...
echo "=== V3.6revC PRODUCTION COMPLETE ==="

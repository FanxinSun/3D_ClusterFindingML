void day3_cmp(){
  ...
  auto d2=[&](TH1*a,TH1*b,const char*xt,const char*ti,bool logy,bool perEv){
    ...
    L->AddEntry(a,"REAL island91","l"); L->AddEntry(b,"SIM island91 (B3)","l"); L->Draw(); };
  ...
  c.cd(6); d2(H(r,"z","a6",100,-320,320),H(s,"z","b6",100,-320,320),"z [cm]","cluster z  [window residual]",true,false);
  ...
  c.cd(9); d2(H(r,"layer","a9",48,6.5,54.5),H(s,"layer","b9",48,6.5,54.5),"TPC layer","clusters/event/layer  [content residual]",true,true);
  c.SaveAs("/home/rog/sPHENIX/3D_ClusterFindingML/sim_validation_plots/day3_final_cmp.png");
  ...
  printf("  %-8s real %8.0f | sim %8.0f | x%.2f  (per event)\n","islands",r->GetEntries()/100.,s->GetEntries()/20.,(s->GetEntries()/20.)/(r->GetEntries()/100.));
  printf("  z-span   real +-%.0f cm | sim +-%.0f cm  (pileup-window limitation of exam5 g4hits)\n",

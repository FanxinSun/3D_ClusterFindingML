...
void cmp_pau(){
  ...
  auto d3=[&](TVirtualPad*p,TH1D*a,TH1D*b,TH1D*g,const char*ti,bool logy,const char*slab="SIM detached pipeline (v3.5revC)"){
  ...
  // (prodclus_v35.root) — same algorithm as the real ntp_cluster curve, so the
  // cluster canvas is apples-to-apples (the old island91 source mixed algorithms;
  // named residual (a) resolved here).
  TFile*fb2=TFile::Open("prodclus_v35.root"); TTree*bc=(TTree*)fb2->Get("ntp_clus");
  TCanvas c2("c2","",1500,900); c2.Divide(3,2);
  const char* RCC="layer>=7&&layer<=54";
  const char* PL="SIM v3.5revC #times ported prod. clusterizer";

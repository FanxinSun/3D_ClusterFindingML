...
void day2_final(){
  ...
  TFile*f=TFile::Open("digi_cal.root"); TTree*h=(TTree*)f->Get("ntp_hit");
  ...
  TFile*id=TFile::Open("island_digi_cal.root"); TTree*tid=(TTree*)id->Get("island");
  printf("== day-2 final numbers ==\n");
  ...
  printf("occupancy: pixels/event real 257k | digi %.0fk (x%.1f)\n",
    d_adc->GetEntries()/20/1000., d_adc->GetEntries()/20./257079.);
  ...
  auto d2=[&](TH1*a,TH1*b,const char*xt,const char*ti,bool logy,bool perEv){
    ...
    L->AddEntry(a,"REAL","l"); L->AddEntry(b,"DIGI B3 (20/11 p0.39 p2 5e-4)","l"); L->Draw(); };
  ...
  c.SaveAs("/home/rog/sPHENIX/3D_ClusterFindingML/sim_validation_plots/digi_day2.png");
  printf("saved digi_day2.png\n");
}

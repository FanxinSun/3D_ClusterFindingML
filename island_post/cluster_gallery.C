...
//   bottom 2 rows SIM = composed v3.6 (B3 readout, 390 kHz).
                  ...
                  const char *simisl = "island_frames_v36.root")
{
  ...
  for (int i = 0; i < NSHOW && i < (int) sh.size(); ++i)
  {
    ...
    sh[i]->Draw("COLZ");
    tx.DrawLatex(0.14, 0.86, Form("SIM pAu v3.6  L%d  n=%.0f  asym=%.2f", ssel[i].layer, ssel[i].size, ssel[i].asym));
  }
  c.SaveAs("/home/rog/sPHENIX/3D_ClusterFindingML/sim_validation_plots/cluster_gallery.png");
  printf("cluster_gallery.png regenerated (real vs v3.6)\n");
}

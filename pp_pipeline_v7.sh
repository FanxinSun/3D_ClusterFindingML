...
#                  chunks + in-generator MBD proxy -> island_post/pp7_mbd.txt
...
# ---------------- parameters ----------------
VER=v7d                   # v7 DRAFT (2026-09-02): world-tuned generator (reshape campaign)
...
RAWPFX=raw_lib_pp7        # v7 library family (world tune); pp61/pp6t/pp55 untouched
...
GEN_SEED_BASE=20260902    # v7 fresh generator seeds
GEN_TUNE=2.55             # v7 world tune W1: pT0Ref (Monash 2.28 default; reshape pick 2026-09-02)
GEN_EXTRA="StringPT:sigma=0.36;StringFlav:probStoUD=0.27;StringFlav:probQQtoQ=0.11"  # v7 world tune W1 extras (gen_pp_v7 argv[6])
...
COMP_SEED_BASE=2026097    # v7 composer seeds
...
for i in $(seq 0 $((NCHUNK-1))); do
  LIBS+="${RAWPFX}_$i.root,"; EVALS+="eval_pp7_$i.root,"; MBDDATS+="pp7_run_$i.dat,"
done
LIBS=${LIBS%,}; EVALS=${EVALS%,}; MBD="pp7_mbd.txt|${MBDDATS%,}"
...
if run_stage gen; then
  ...
  cd "$GEN"
  if [ ! -x gen_pp_v7 ] || [ gen_pp.cc -nt gen_pp_v7 ]; then
    g++ -O2 -std=c++$(root-config --cxxstandard) gen_pp.cc -o gen_pp_v7 \
        -I install/include -L install/lib -lpythia8 -ldl \
        -Wl,-rpath,"$GEN/install/lib"
    echo "gen_pp_v7 compiled"
  fi
  for i in $(seq 0 $((NCHUNK-1))); do
    ./gen_pp_v7 $GEN_PER pp7_run_$i.dat $((GEN_SEED_BASE+i)) $GEN_TUNE 0 "$GEN_EXTRA" \
        > "$LOGS/gen_pp7_$i.log" 2> fired_pp7_$i.txt &
  done
  wait
  # plain `wait` returns 0 even if a background job died — verify explicitly
  NDONE=$(grep -hl GEN-DONE "$LOGS"/gen_pp7_*.log 2>/dev/null | wc -l)
  [ "$NDONE" -eq "$NCHUNK" ] || { echo "[gen] FAILED: only $NDONE/$NCHUNK chunks completed (see $LOGS/gen_pp7_*.log)"; exit 1; }
  for i in $(seq 0 $((NCHUNK-1))); do
    [ -s pp7_run_$i.dat ] || { echo "[gen] FAILED: pp7_run_$i.dat empty/missing"; exit 1; }
  done
  grep -h GEN-DONE "$LOGS"/gen_pp7_*.log
  ...
  { echo "# file event_index_in_file north south fired"
    for i in $(seq 0 $((NCHUNK-1))); do
      awk -v f="pp7_run_$i.dat" '$1=="FIRED"{print f, $2-1, $3, $4, $5}' fired_pp7_$i.txt
    done; } > "$IP/pp7_mbd.txt"
  echo "[gen] DONE: $(grep -c "^pp7_run" "$IP/pp7_mbd.txt") events in pp7_mbd.txt," \
       "fired frac $(awk '$1!~/^#/{n++;s+=$5} END{printf "%.3f", s/n}' "$IP/pp7_mbd.txt")"
fi
...
if run_stage g4; then
  ...
  for i in $(seq 0 $((NCHUNK-1))); do
    ./standalone_tpc sphenix_p5.gdml angantyr/pp7_run_$i.dat PP7_g4hit_$i.root \
        $GEN_PER "$FM" > "$LOGS/pp7_g4_$i.log" 2>&1 &
    [ $(( (i+1) % G4_PAR )) -eq 0 ] && wait
  done
  wait
  for i in $(seq 0 $((NCHUNK-1))); do
    [ -s PP7_g4hit_$i.root ] || { echo "[g4] FAILED: PP7_g4hit_$i.root empty/missing (see $LOGS/pp7_g4_$i.log)"; exit 1; }
    grep -q "TpcSD: wrote" "$LOGS/pp7_g4_$i.log" || { echo "[g4] FAILED: chunk $i did not finish (see $LOGS/pp7_g4_$i.log)"; exit 1; }
  done
  echo "[g4] DONE: $(ls -1 PP7_g4hit_*.root | wc -l) g4hit files"
fi
...
if run_stage gate; then
  ...
  for i in $(seq 0 $((NCHUNK-1))); do
    root -l -b -q -e "gROOT->ProcessLine(\".L tpc_digitize.C+\");
      tpc_transport(\"$P5/PP7_g4hit_$i.root\",\"${RAWPFX}_$i.root\",$GEN_PER,$SIGMA0,$KPRF,\"$FIELD\",\"$TWIST\");
      tpc_readout(\"${RAWPFX}_$i.root\",\"cens_pp7_$i.root\",$GAIN,20.0,1,1,4711,\"$DM\",11.0,0.39,0.55,1.25,0.016,7.0,36.0,70.0,11.0,940.0,2,0.29,10.0,1.24,1.06,-1.0,5.0,0.0);" \
      > "$LOGS/pp7_gate_$i.log" 2>&1
    echo "[gate] chunk $i transported+censused"
  done
  cat > "$LOGS/pp7_census.C" <<'EOM'
void pp7_census(const char *fn, const char *chunk, const char *out)
{
...
EOM
  rm -f pp7_census.txt
  for i in $(seq 0 $((NCHUNK-1))); do
    root -l -b -q "$LOGS/pp7_census.C(\"cens_pp7_$i.root\",\"$i\",\"pp7_census.txt\")" > /dev/null 2>&1
  done
  python3 - "$ANCHOR" <<'EOP'
import sys
anchor = float(sys.argv[1])
fired = {}
for L in open("pp7_mbd.txt"):
    if L.startswith("#"): continue
    f, e, n, s, fl = L.split()
    fired[(f.split("_")[-1].split(".")[0], int(e))] = int(fl)
cnt = {}
for L in open("pp7_census.txt"):
    c, e, n = L.split()
    cnt[(c, int(e))] = int(n)
...
if run_stage prod; then
  ...
  cd "$IP"
  for i in $(seq 0 $((NCHUNK-1))); do
    ln -sf "$P5/PP7_g4hit_$i.root" eval_pp7_$i.root   # identity numbering: g4hit IS the eval
  done
  ...
  for i in $(seq 0 $((NB-1))); do
    ...
    for c in $(seq 0 $((NCHUNK-1))); do
      ...
      LIBS_B+="${RAWPFX}_$c.root,"; EVALS_B+="eval_pp7_$c.root,"; MBD_B+="pp7_run_$c.dat,"
    done
    LIBS_B=${LIBS_B%,}; EVALS_B=${EVALS_B%,}; MBD_B="pp7_mbd.txt|${MBD_B%,}"
    root -l -b -q -e "

...
#include <DisplayOn.C>
#include <G4_Mbd.C>
// P5-pilot strip: #include <G4_CaloTrigger.C>
// P5-pilot strip: #include <G4_Centrality.C>
// P5-pilot strip: #include <G4_DSTReader.C>
#include <G4_Global.C>
// P5-pilot strip: #include <G4_HIJetReco.C>
#include <G4_Input.C>
// P5-pilot strip: #include <G4_Jets.C>
// P5-pilot strip: #include <G4_KFParticle.C>
// P5-pilot strip: #include <G4_ParticleFlow.C>
#include <G4_Production.C>
// P5-pilot strip: #include <G4_TopoClusterReco.C>
...
//  Enable::INTT_SUPPORT = true; // enable global support structure readout
 ...
 //Enable::GLOBAL_FASTSIM = true;
  ...
  // P5 strip: Enable::CALOTRIGGER = Enable::CEMC_TOWER && Enable::HCALIN_TOWER && Enable::HCALOUT_TOWER && false;

  // P5 strip: Enable::JETS = (Enable::GLOBAL_RECO || Enable::GLOBAL_FASTSIM) && true;
  // P5 strip: Enable::JETS_EVAL = Enable::JETS && true;
  // P5 strip: Enable::JETS_QA = Enable::JETS && Enable::QA && true;

  // HI Jet Reco for p+Au / Au+Au collisions (default is false for
  // single particle / p+p-only simulations, or for p+Au / Au+Au
  // simulations which don't particularly care about jets)
  // P5 strip: Enable::HIJETS = Enable::JETS && Enable::CEMC_TOWER && Enable::HCALIN_TOWER && Enable::HCALOUT_TOWER && false;

  // 3-D topoCluster reconstruction, potentially in all calorimeter layers
  // P5 strip: Enable::TOPOCLUSTER = Enable::CEMC_TOWER && Enable::HCALIN_TOWER && Enable::HCALOUT_TOWER && false;
  // particle flow jet reconstruction - needs topoClusters!
  // P5 strip: Enable::PARTICLEFLOW = Enable::TOPOCLUSTER && true;
  // centrality reconstruction
  // P5 strip: Enable::CENTRALITY = true;
  ...
  // P5 offline: serve every payload from the local CDB_offline mirror
  // (no BNL network; nopayloadclient would abort on the empty catalog reply).
  CDBInterface::instance()->ReadCalibrationsFromFile("/home/rog/sPHENIX/3D_ClusterFindingML/P5/cdb_local.txt");
  //---------------
  // World Settings
  //---------------
...
//  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar field strength of 1.4T
  ...
  // P5 strip: if (false)  // P5 strip: Enable::CENTRALITY
  {
      ; // P5 strip
  }
  ...
  // P5 strip: if (Enable::CALOTRIGGER)
  {
    // P5 strip: CaloTrigger_Sim();
  }
  ...
  // P5 strip: if (Enable::JETS) Jet_Reco();
  // P5 strip: if (Enable::HIJETS) HIJetReco();
  ...
  // P5 strip: if (Enable::JETS_EVAL) Jet_Eval(outputroot + "_g4jet_eval.root");
  ...
  // P5 strip: if (Enable::JETS_QA) Jet_QA();
